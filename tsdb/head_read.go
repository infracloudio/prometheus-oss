...
// The caller must ensure that s.ooo is not nil.
func (s *memSeries) oooMergedChunk(meta chunks.Meta, cdm *chunks.ChunkDiskMapper, mint, maxt int64, oooMeta *OOOChunkMeta) (chunk *mergedOOOChunks, err error) {
	...
	for i, c := range s.ooo.oooMmappedChunks {
		...
		if chunkRef > oooMeta.OOOLastRef {
			break
		}
	...
	for _, c := range tmpChks {
		...
		if c.meta.Ref == oooHeadRef {
			...
			// it as it is.
			if s.ooo.oooHeadChunk.minTime == oooMeta.OOOLastMinTime && s.ooo.oooHeadChunk.maxTime == oooMeta.OOOLastMaxTime {
				...
				xor, err = s.ooo.oooHeadChunk.chunk.ToXORBetweenTimestamps(oooMeta.OOOLastMinTime, oooMeta.OOOLastMaxTime)
			}
			if err != nil {
			...
			if c.meta.Ref == oooMeta.OOOLastRef &&
				(c.origMinT != oooMeta.OOOLastMinTime || c.origMaxT != oooMeta.OOOLastMaxTime) {
				...
				// markers.
				c.meta.Chunk = boundedChunk{chk, oooMeta.OOOLastMinTime, oooMeta.OOOLastMaxTime}
			} else {
				c.meta.Chunk = chk
			}
		}
